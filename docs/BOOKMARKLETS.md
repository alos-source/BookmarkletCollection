...
---
### Amazon To Keepa
*Opens the Keepa price tracking page for the current Amazon product.*

👉 [Install Amazon To Keepa](javascript:(function()%7Bvar%20url=window.location.href;var%20asin=%22%22;var%20urlMatch=url.match(/%5C/(%3F:dp%7Cgp%5C/product)%5C/(%5BA-Z0-9%5D%7B10%7D)(%3F:%5C/%7C%5C%3F%7C%24)/);if(urlMatch)%7Basin=urlMatch%5B1%5D;%7Delse%7Bvar%20asinInput=document.getElementById(%22ASIN%22)%7C%7Cdocument.querySelector(%27%5Bdata-asin%5D%27);asin=asinInput%3F(asinInput.value%7C%7CasinInput.getAttribute(%27data-asin%27)):%22%22;%7D%0Aif(%21asin)%7Balert(%22Keine%20Amazon%20ASIN%20gefunden.%20Bist%20du%20auf%20einer%20Produktseite%3F%22);return;%7D%0Avar%20domainId=%221%22;if(url.indexOf(%22.de%22)%21==-1)domainId=%223%22;else%20if(url.indexOf(%22.co.uk%22)%21==-1)domainId=%222%22;else%20if(url.indexOf(%22.fr%22)%21==-1)domainId=%224%22;else%20if(url.indexOf(%22.it%22)%21==-1)domainId=%228%22;else%20if(url.indexOf(%22.es%22)%21==-1)domainId=%229%22;var%20keepaUrl=%22https://keepa.com/%23%21product/%22%2BdomainId%2B%22-%22%2Basin;window.open(keepaUrl,%27_blank%27);%7D)();)

---
### Humble Bulk Downloader
*Downloads all available zip files from a Humble Bundle page in bulk.*

👉 [Install Humble Bulk Downloader](javascript:(function()%7Bconst%20links=Array.from(document.querySelectorAll(%27a%27));const%20downloadUrls=%5B%5D;const%20seen=new%20Set();links.forEach(a=%3E%7Bconst%20href=a.href%7C%7C%22%22;const%20text=(a.innerText%7C%7C%22%22).toUpperCase().trim();const%20isHumbleCdn=href.includes(%27cdn.humble.com%27);const%20isZipText=text===%27ZIP%27;const%20isDownloadText=text===%27HERUNTERLADEN%27%7C%7Ctext===%27DOWNLOAD%27;if(href%26%26(isHumbleCdn%7C%7CisZipText%7C%7CisDownloadText)%26%26%21seen.has(href))%7Bif(href.startsWith(%27http%27))%7Bseen.add(href);downloadUrls.push(href);%7D%7D%7D);if(downloadUrls.length===0)%7Balert(%22Keine%20Download-Links%20gefunden%21%22);return;%7D%0Aif(%21confirm(downloadUrls.length%2B%22%20Downloads%20gefunden.%20M%C3%B6chtest%20du%20den%20Bulk-Download%20starten%3F%5Cn%5Cn(Hinweis:%20Erlaube%20Pop-ups%20f%C3%BCr%20diese%20Seite,%20falls%20der%20Browser%20blockiert.)%22))%7Breturn;%7D%0AdownloadUrls.forEach((url,index)=%3E%7BsetTimeout(()=%3E%7Bconsole.log(%22Starte%20Download%20%22%2B(index%2B1)%2B%22:%20%22%2Burl);const%20iframe=document.createElement(%27iframe%27);iframe.style.display=%27none%27;iframe.src=url;document.body.appendChild(iframe);setTimeout(()=%3Edocument.body.removeChild(iframe),30000);%7D,index%2A1500);%7D);alert(%22Bulk-Download%20gestartet.%20Bitte%20lass%20den%20Tab%20offen,%20bis%20alle%20Dateien%20angefordert%20wurden.%22);%7D)();)
...
---
### Rss Finder
*Finds and displays RSS feeds on the current page.*

👉 [Install Rss Finder](javascript:(function()%7Bconst%20feeds=%5B%5D;const%20seen=new%20Set();const%20metaLinks=document.querySelectorAll(%27link%5Btype%2A=%22rss%22%5D,%20link%5Btype%2A=%22atom%22%5D,%20link%5Btype%2A=%22xml%22%5D%5Brel=%22alternate%22%5D%27);metaLinks.forEach(link=%3E%7Bconst%20url=link.href;if(url%26%26%21seen.has(url))%7Bseen.add(url);feeds.push(%7Btitle:link.title%7C%7C%22RSS/Atom%20Feed%22,url:url,type:%22Header%20Meta%22%7D);%7D%7D);const%20anchors=document.querySelectorAll(%27a%27);anchors.forEach(a=%3E%7Bconst%20url=a.href;const%20rel=(a.getAttribute(%27rel%27)%7C%7C%22%22).toLowerCase();const%20text=a.innerText.trim();const%20textLower=text.toLowerCase();const%20isFeedUrl=/%5C/(feed%7Crss%7Catom%7Crdf)s%3F(%5C/%7C%24%7C%5C.%7C%5C%3F)/i.test(url);const%20isFeedRel=rel.includes(%27alternate%27);const%20isShortFeedText=text.length%3C15%26%26(textLower.includes(%27rss%27)%7C%7CtextLower===%27atom%27%7C%7CtextLower===%27feed%27%7C%7CtextLower===%27feeds%27);if(url%26%26url.startsWith(%27http%27)%26%26%21seen.has(url))%7Bif(isFeedUrl%7C%7CisFeedRel%7C%7CisShortFeedText)%7Bif(textLower.includes(%27atom%27)%26%26text.length%3E20%26%26%21isFeedUrl)return;seen.add(url);feeds.push(%7Btitle:text.substring(0,50)%2B(text.length%3E50%3F%22...%22:%22%22),url:url,type:isFeedRel%3F%22Standard-Link%22:%22Muster-Match%22%7D);%7D%7D%7D);if(feeds.length===0)%7Balert(%22Kein%20RSS-Feed%20gefunden.%22);return;%7D%0Aconsole.table(feeds);if(feeds.length===1)%7Bif(confirm(%60Feed%20gefunden:%5Cn%5Cn%24%7Bfeeds%5B0%5D.title%7D%5Cn%24%7Bfeeds%5B0%5D.url%7D%5Cn%5CnKopieren%3F%60))%7Bnavigator.clipboard.writeText(feeds%5B0%5D.url);%7D%7Delse%7Bconst%20list=feeds.map((f,i)=%3E%60%24%7Bi%2B1%7D.%24%7Bf.title%7D:%24%7Bf.url%7D%60).join(%27%5Cn%5Cn%27);alert(%60%24%7Bfeeds.length%7DFeeds%20gefunden:%5Cn%5Cn%24%7Blist%7D%60);%7D%7D)();)

---
### Site Shot
*Captures a screenshot of the current page and downloads it as an image.*

👉 [Install Site Shot](javascript:(function()%7Bconst%20script=document.createElement(%27script%27);script.src=%27https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js%27;script.onload=function()%7Bhtml2canvas(document.body).then(canvas=%3E%7Bconst%20link=document.createElement(%27a%27);const%20timestamp=new%20Date().toISOString().replace(/%5B:.%5D/g,%27-%27).slice(0,16);link.download=%60screenshot_%24%7Btimestamp%7D.png%60;link.href=canvas.toDataURL(%22image/png%22);link.click();alert(%22Screenshot%20captured%20and%20download%20started%21%22);%7D);%7D;document.head.appendChild(script);%7D)();)
...
---
### Yt To Socialblade
*Opens the Socialblade page for the YouTube channel handle of the current video or channel page.*

👉 [Install Yt To Socialblade](javascript:(function()%7Bvar%20url=window.location.href;var%20handle=%22%22;var%20channelMatch=url.match(/%5C/(%3F:%5C/)%3F(%40%5Ba-zA-Z0-9%5C._%5C-%5D%2B)/);var%20channelLink=document.querySelector(%27ytd-video-owner-renderer%20a%5Bhref%2A=%22/%40%22%5D%27);if(channelMatch%26%26%21url.includes(%27/watch%27))%7Bhandle=channelMatch%5B1%5D;%7Delse%20if(channelLink)%7Bvar%20href=channelLink.getAttribute(%27href%27);var%20handleMatch=href.match(/(%40%5Ba-zA-Z0-9%5C._%5C-%5D%2B)/);if(handleMatch)handle=handleMatch%5B1%5D;%7D%0Aif(%21handle)%7Bvar%20metaMatch=document.documentElement.innerHTML.match(/%22%40(%5Ba-zA-Z0-9%5C._%5C-%5D%2B)%22/);if(metaMatch)handle=%22%40%22%2BmetaMatch%5B1%5D;%7D%0Aif(%21handle)%7Balert(%22Kein%20YouTube-Handle%20gefunden.%20Bist%20du%20auf%20einer%20Video-%20oder%20Kanalseite%3F%22);return;%7D%0Avar%20sbUrl=%22https://socialblade.com/youtube/handle/%22%2Bhandle;window.open(sbUrl,%27_blank%27);%7D)();)
